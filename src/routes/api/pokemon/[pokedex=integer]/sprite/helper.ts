...
import { PokeapiVersionNames, type IGameGroups } from '$/lib/data/games';
import type { IPokemon, ISprites } from '$/lib/types/IPokemon';

export function getSpriteAndInfo(
	sprite: any,
	wantsShiny: boolean,
	wantsFemale: boolean,
	wantsBack: boolean
...
export function getSpriteForGameAnimation(
	pokemon: IPokemon,
	...
	if (selectedGame) {
		...
		if (selectedGame.pokeapi === 'black-2-white-2') {
			pokeapiSpriteName = 'black-white';
		} else if (selectedGame.pokeapi === 'gold-silver') {
			pokeapiSpriteName = PokeapiVersionNames.GOLD;
		} else if (selectedGame.pokeapi === 'sun-moon') {
			pokeapiSpriteName = 'ultra-sun-ultra-moon';
		}

		if (selectedGame.pokeapi === 'home') {
			...
			// eslint-disable-next-line @typescript-eslint/ban-ts-comment
			// @ts-ignore TODO: Can't be asked to type this properly
			const generationSpecific = pokemon.sprites.versions
				? pokemon.sprites.versions[selectedGame.generation.pokeApiName]
				: undefined;
