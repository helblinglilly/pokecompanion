...
export async function GET({ request, platform, cookies, params }) {
    ...
    if (userPreferences.selectedGame){
        body.effect_changes.forEach((change) => {
            ...
            const changeGameIndex = GameGroups.findIndex((group) => group.pokeapi === changeGame?.pokeapi);

            const selectedGameIndex = GameGroups.findIndex((group) => group.pokeapi === userPreferences.selectedGame?.pokeapi);

            change.effect_entries.forEach((entry) => {
                console.log(entry);
                const toReplaceIndex = effect_entries.findIndex((entry) => entry.language.name === entry.language.name);
                if (changeGameIndex > -1 && selectedGameIndex > -1 && toReplaceIndex > -1){
                    console.log(changeGameIndex, selectedGameIndex, toReplaceIndex)
                    if (changeGameIndex <= selectedGameIndex){
