...
{#if $page.data.session}
	...
	<div class="columns">
		<div class="column">
			<p>Hi {$page.data.session.user?.name ?? 'User'}<strong /></p>
			<p>You're signed in with {$page.data.session.user.email}</p>
		</div>
	</div>
...
{:else}
	...
	<div class="columns">
		<div class="column centered">
			<button id="googleButton" class="button" on:click={() => signIn('google')}>
				<svg
	...
	<div class="columns">
		<div class="column centered">
			<button id="githubButton" class="button" on:click={() => signIn('github')}>
				<svg
					class="svelte-fa icon svelte-1cj2gr0"
					style="height:1em;vertical-align:-.125em;transform-origin:center;overflow:visible;"
					viewBox="0 0 496 512"
