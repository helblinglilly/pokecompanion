import { getGameGroupFromName, type PokeapiVersionGroups } from "../data/games";
...
function adjustMoveForGame(move: IMove, versionGroupName: PokeapiVersionGroups){
	...
	if (selectedGame.generation.number <= 3){
		adjusted.damage_class.name = getLegacyDamageClass(adjusted.damage_class.name, adjusted.type.name)
	}
